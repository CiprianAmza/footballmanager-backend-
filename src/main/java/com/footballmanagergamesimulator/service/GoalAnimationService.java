...
public class GoalAnimationService {
    ...
    public GoalAnimationData generate(
        ...
        // 7. Build events list
        List<AnimationEvent> events = buildEvents(chain, scorer, passFrames);

        // 8. Goal target Y (where the ball ends up in the net)
        double goalY = 40 + rng.nextDouble() * 20;
        ...
        long defGkId = def11.stream()
                ...
                .map(Human::getId)
                .findFirst().orElse(0L);
        double gkDiveTargetY = goalY + (rng.nextDouble() < 0.5 ? -14 : 14); // dives wrong way
        ...
        for (int f = 0; f <= TOTAL_FRAMES; f++) {
            ...
            for (Human p : allPlayers) {
                ...
                if (!isCarrier && dist > 3 && dist < 35 && group != 0) {
                    ...
                    // Attacking players attract more strongly
                ...
                double finalX = clamp(bx + patrolX + attractX + runX, 1, 99);
    ...
    private List<AnimationEvent> buildEvents(List<Human> chain, Human scorer, int[] passFrames) {
        List<AnimationEvent> events = new ArrayList<>();
        ...
        goal.setFrame(148);
        goal.setType("GOAL");
        goal.setFromPlayerId(scorer.getId());
        events.add(goal);
