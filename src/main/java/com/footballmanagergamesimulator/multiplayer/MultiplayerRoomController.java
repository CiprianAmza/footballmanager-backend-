...
public class MultiplayerRoomController {
    ...
    @PostMapping("/room/continue") public Map<String,Object> continueDay() { boolean claimed = coordinator.cast(service.user().getId(), VoteSource.MANUAL); if (claimed) coordinator.advanceClaimed(); return state(false); }
    @DeleteMapping("/room/continue") public Map<String,Object> withdraw() { coordinator.withdraw(service.user().getId()); return state(false); }
    @PostMapping("/room/fast-forward") public Map<String,Object> fastForward(@RequestBody FastForwardRequest r) { toggleFastForward(r.enabled(), r.seasons()); if (r.enabled() && service.members(service.requireMemberRoom()).stream().allMatch(GameRoomMember::isFastForwardEnabled)) { boolean claimed = coordinator.cast(service.user().getId(), VoteSource.FAST_FORWARD); if (claimed) coordinator.advanceClaimed(); } return state(false); }
    @DeleteMapping("/room/fast-forward") public Map<String,Object> stopFastForward() { toggleFastForward(false, 0); return state(false); }
    @GetMapping("/room/state") public Map<String,Object> stateEndpoint() { return state(true); }

    private void toggleFastForward(boolean enabled, int seasons) {
        GameRoom room = service.requireMemberRoom(); GameRoomMember m = service.member(service.user().getId()); m.setFastForwardEnabled(enabled); if (enabled) { GameCalendar c = calendars.findTopByOrderBySeasonDesc().orElseThrow(); m.setFastForwardUntilAbsoluteDay((long)(c.getSeason() + Math.max(1, seasons)) * 366L); } else { m.setFastForwardUntilAbsoluteDay(null); RoomContinueCycle cycle = service.currentCycle(room); if (cycle != null) service.votes().findByCycleIdAndUserId(cycle.getId(), m.getUserId()).filter(v -> v.getSource() == VoteSource.FAST_FORWARD).ifPresent(service.votes()::delete); } service.membersRepository().save(m);
    }
