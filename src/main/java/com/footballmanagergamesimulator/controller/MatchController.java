...
public class MatchController {
    ...
    /**
     * Faza C: incremental fetch of the presentational possession chains
     * (MatchPhaseData) generated by MatchPhaseEngine. The frontend passes the
     * first minute it has NOT yet rendered and receives only the new phases,
     * instead of re-reading the cumulative LiveMatchData payload.
     */
    @GetMapping("/live/{key}/phases")
    public org.springframework.http.ResponseEntity<java.util.List<com.footballmanagergamesimulator.frontend.MatchPhaseData>> getLiveMatchPhases(
            @PathVariable String key,
            @RequestParam(value = "fromMinute", defaultValue = "0") int fromMinute) {
        LiveMatchSession session = liveMatchSimulationService.getSessionOrRecover(key);
        if (session == null) return org.springframework.http.ResponseEntity.notFound().build();
        requireLiveMatchOwnership(session);
        return org.springframework.http.ResponseEntity.ok(session.phasesSince(fromMinute));
    }
