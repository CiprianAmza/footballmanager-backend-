-- Press Conference V2 — cross-database migration contract (PostgreSQL).
-- Runtime datasource is H2; this migration exists so the PostgreSQL schema
-- contract (FlywayPhase0CrossDatabaseIT / PostgreSqlGameSaveIT) stays in parity.
-- The tables are additive and leave the legacy press_conference table untouched.
...
CREATE INDEX IF NOT EXISTS idx_pc_session_team_season ON press_conference_session (team_id, season_number);
CREATE INDEX IF NOT EXISTS idx_pc_session_status ON press_conference_session (status);
...
CREATE INDEX IF NOT EXISTS idx_pc_question_session ON press_conference_question (session_id, order_index);
...
CREATE INDEX IF NOT EXISTS idx_pc_answer_session ON press_conference_answer (session_id);
