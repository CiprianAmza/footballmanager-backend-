...
UPDATE users SET email = CONCAT('legacy-', id, '@invalid.local') WHERE email IS NULL OR BTRIM(email) = '';
UPDATE users SET roles = 'USER' WHERE roles IS NULL OR BTRIM(roles) = '';
UPDATE users SET career_role = 'MANAGER' WHERE career_role IS NULL OR BTRIM(career_role) = '';
...
SELECT u.id, u.manager_id, 'MANAGER', 'USER', COALESCE(NULLIF(BTRIM(u.username), ''), CONCAT('user-', u.id)), TRUE, FALSE
FROM users u
...
SELECT u.id, COALESCE(NULLIF(BTRIM(u.career_role), ''), 'MANAGER'), 'USER',
       COALESCE(NULLIF(BTRIM(u.username), ''), CONCAT('user-', u.id)), TRUE, FALSE
FROM users u
...
SELECT h.id, CASE WHEN h.type_id = 4 THEN 'MANAGER' ELSE 'PLAYER' END, 'AI',
       COALESCE(NULLIF(BTRIM(h.name), ''), CONCAT('human-', h.id)), NOT h.retired, h.retired
FROM human h
