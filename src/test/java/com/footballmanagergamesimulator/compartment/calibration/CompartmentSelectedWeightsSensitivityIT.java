...
class CompartmentSelectedWeightsSensitivityIT {
    @Test void paceShadowStrikerMoraleStayForwardAndDefensiveMentalityAreReported() throws Exception {
        ...
        List<Experiment> experiments = List.of(
                ...
                new Experiment("shadow-striker-measurement-only", base.baselineTeam().withShadowStriker(), "compartment.roles.ADVANCED_PLAYMAKER.attack"),
        ...
        for (Experiment experiment : experiments) {
            ...
            if (experiment.id().contains("shadow-striker")) result = result.withRoleClassification(true, false);
            results.add(result);
