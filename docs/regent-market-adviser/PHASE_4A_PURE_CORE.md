...
| --- | --- | --- |
| `SAFE_COMPANY` | `direction * U(0, maximumDailyMagnitude)` | magnitude is in `0..1%`; the direction has configurable positive probability but may be negative |
| `SPECULATIVE` | `return = U(0,1) - 0.50`; `close=max(open * (1+return), floor)` | return is hard-bounded to `[-50%, +50%]`; floor is strictly positive |
...
Prices and returns use `BigDecimal`, so multiplying a legitimate amount around `Long.MAX_VALUE` cannot overflow a `long`. The current speculative generator is continuous and can approach either endpoint; the endpoints remain part of the contractual inclusive bound for versioned implementations.

## Adviser limits

`TraderAdviser` holds immutable skill, reputation, daily salary, contract period and model version. `TraderAdviceModel` consumes only an `AdviserSignal` made from observed trailing return and volatility. It does not accept a future return, quote result, mutable account, holdings, supply, or execution service.
